...
void
check_arrivals(process **ready_queue, task **global_tasks, int cur_time, int nproc, int *pid_count)
{
    for(int i = 0; i < nproc; i++)
    {
...
void
schedule_mllf(process **rdqueue, int nproc, int hyperperiod, task **global_tasks, int *pid_count)
{
    int cur_time = 0;
    int prev_task_id = -1;
    int cur_task_id = -1;
